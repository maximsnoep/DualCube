...
impl Layout {
    ...
    pub fn place_paths(&mut self) -> Result<(), PropertyViolationError> {
        ...
        while let Some(edge_id) = edge_queue.pop_front() {
            ...
            let ccs = hutspot::graph::find_ccs(&primal.structure.face_ids(), |face_id| {
                primal
                    .structure
                    .neighbors(face_id)
                    .into_iter()
    ...
    pub fn assign_patches(&mut self) {
        ...
        let patches = hutspot::graph::find_ccs(&self.granulated_mesh.face_ids(), |face_id: FaceID| face_to_neighbors[&face_id].clone());
