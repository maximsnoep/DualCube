...
use log::info;
use mehsh::integrations::tetgen::TetgenCliParams;
use mehsh::mesh::connectivity::HasVertices;
...
impl Hex {
    pub fn from_solution(solution: &Solution) -> io::Result<Self> {
        ...
        let tet_export = mesh.to_tet(
            &tet_mesh_path1,
            &tet_mesh_path2,
            &TetgenCliParams {
                command: std::env::var("TETGEN_COMMAND").unwrap_or_else(|_| "tetgen".to_owned()),
                args: vec![std::env::var("TETGEN_OPTIONS").unwrap_or_else(|_| "-pY".to_owned())],
            },
        )?;
        info!(
